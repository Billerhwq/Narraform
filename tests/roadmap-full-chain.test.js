...
test('总体闭环：素材 → 策略 → 内容 → 草稿回执 → 表现 → 经验 → 下一次策略', async () => {
  ...
  const capturedAt = new Date(new Date(receipt.receipt.submittedAt).getTime() + 48 * 60 * 60 * 1000).toISOString();
  const snapshot = await json('/api/performance-snapshots', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contentId, contentRevision: 1, receiptId: receipt.receipt.receiptId, platform: 'xiaohongshu', goal: 'save', contentType: 'product_marketing', capturedAt, source: 'manual', metrics: { impressions: 18420, reads: 15107, likes: 612, saves: 941, comments: 83 } }) });

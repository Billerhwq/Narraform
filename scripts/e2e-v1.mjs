...
const initialContents = await (await fetch('http://127.0.0.1:4176/api/contents')).json();
const initialContentIds = new Set((initialContents.contents || []).map((item) => item.id));
...
const sourceIsolated = !/README|根据.*(?:资料|文档|文件)|基于.*(?:资料|文档|文件)|你提供的资料/i.test(body);
  ...
  await page.locator('.operation-result-bar').waitFor({ state: 'visible', timeout: 150000 });
  ...
  await page.locator('.operation-result-bar').waitFor({ state: 'detached', timeout: 5000 });
  ...
  await page.locator('.operation-result-bar').waitFor({ state: 'visible', timeout: 150000 });
  await page.locator('.global-save-state.is-saved').waitFor({ timeout: 15000 });
  await page.waitForFunction(() => {
    const button = document.querySelector('.body-regenerate');
    const editor = document.querySelector('.rich-editor:not(.is-candidate)');
    return button && !button.disabled && editor && !editor.classList.contains('is-readonly');
...
const bodyRefreshPreservesTitles = JSON.stringify(titlesAfterBodyRefresh) === JSON.stringify(titlesBeforeBodyRefresh);
  ...
  await page.locator('.global-save-state.is-pending').waitFor({ timeout: 5000 });
  await page.waitForFunction(() => document.querySelector('.global-save-state')?.classList.contains('is-saved') || document.querySelector('.global-save-state')?.classList.contains('is-error'), undefined, { timeout: 15000 });
  const manualSaveState = await page.locator('.global-save-state').innerText();
  if (manualSaveState.includes('失败')) throw new Error('手工编辑后自动保存失败');
...
for (const item of contents.contents) if (!initialContentIds.has(item.id)) await fetch(`http://127.0.0.1:4176/api/contents/${item.id}`, { method: 'DELETE' });

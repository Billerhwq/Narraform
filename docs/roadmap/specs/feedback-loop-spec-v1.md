...
- 绑定回执时，`ageHours` 必须由 `capturedAt - submittedAt` 计算，不接受前端传入的固定小时数。
- 未绑定回执时，用户必须提供大于或等于 0 的已发布小时数。
- `dataQuality` 只允许 `complete | partial | unverified`。
- 快照必须显式返回 `publicationLinkStatus=verified | pending | unlinked`。

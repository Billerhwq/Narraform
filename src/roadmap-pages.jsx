...
export function PublishWorkspace({ onCreateContent }) {
  ...
  const cancelDelivery = async () => {
    if (!job) return;
    try {
      const data = await request(`/api/delivery-jobs/${job.jobId}/cancel`, { method: 'POST' });
      setJob(data.job);
      Message.info('发布任务已取消，已完成的平台不受影响');
    } catch (error) { Message.error(error.message); }
  };
